...
function branchChildren(branchId: string, t: (k: string, fallback: string) => string): INavData[] {
  ...
  return [
    ...
    { name: t('nav.studyMaterial', 'Study Material'), url: `/branches/${branchId}/study-materials`, iconComponent: { name: 'cil-description' }, linkProps: exact },
...
export function buildNav(me: Me | null, i18n: I18nService): INavData[] {
  ...
  if (me.role === 'STUDENT') {
    return [
      ...
      { name: t('nav.studyMaterial', 'Study Material'), url: '/study-materials', iconComponent: { name: 'cil-description' }, linkProps: exact },
